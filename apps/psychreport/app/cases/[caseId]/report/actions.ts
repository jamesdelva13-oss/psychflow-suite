...
export async function draftSection(formData: FormData): Promise<void> {
  ...
  // The rendered blocks come from the generation itself — the same
  // computation that described them to the model — so the document the model
  // was told about and the document the clinician sees are one object.
  const written = await persistGeneration(svc, {
    ...
    tables: result.section.renderedBlocks,
  });
