...
function inv(over: Partial<InvitationRow> = {}): InvitationRow {
  return {
    ...
    bank_id: "teacher-intake", bank_version: "1.1.2", token_hash: "h",
...
/** Build a complete, valid teacher submission with the safety gate set. */
function completeDrafts(gate: "yes" | "no"): Draft[] {
  const r: ResponseMap = { "TCH-CORE-008": ["behavior"], "TCH-BEH-002": gate };
  for (let i = 0; i < 40; i++) {
  ...
  r["TCH-BEH-002"] = gate; // keep the gate explicit even if the loop touched it
...
test("submitting a completed invitation is rejected (409)", async () => {
  const { svc } = fakeSvc(completeDrafts("no"));
...
test("complete submit with safety gate = yes → 200, priority flag + locked source", async () => {
  const { svc, calls } = fakeSvc(completeDrafts("yes"));
  const r = await submitResponse({ svc, sessionInvitationId: "A", inv: inv(), bank });
...
test("complete submit with safety gate = no → no priority flag", async () => {
  const { svc, calls } = fakeSvc(completeDrafts("no"));
  const r = await submitResponse({ svc, sessionInvitationId: "A", inv: inv(), bank });
  assert.equal(r.status, 200);
